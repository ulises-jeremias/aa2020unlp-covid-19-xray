    ...
    for (x_split, y_split), split_name in sets:
        ...
        for img, label in zip(x_split, y_split):
            ...
            dest = os.path.sep.join(
                [split_dir, label, "{0:04d}.jpg".format(i)])
...
def create_dataset(dataset, version, data_dir, split, splits_dir, **kwargs):
    data_dir = data_dir if data_dir else os.path.sep.join(
        ["./data", dataset, "data"])
    ...
    train_split = (load_from_split(
        version, data_dir, train_split_path), 'train')
